...
const Float32 = @import("BinaryStream").Float32;
...
pub const FloatTag = struct {
    ...
    pub fn deinit(self: FloatTag, allocator: std.mem.Allocator) void {
        if (self.name) |n| {
            allocator.free(n);
        }
    }

    pub fn toJSON(self: FloatTag) f32 {
        return self.value;
    }

    pub fn read(stream: *BinaryStream, allocator: std.mem.Allocator, options: ReadWriteOptions) !FloatTag {
        if (options.tag_type) {
            const read_type = try Int8.read(stream);
            if (read_type != @intFromEnum(TagType.Float)) {
                return error.InvalidTagType;
            }
        }

        const name = try readName(stream, allocator, options);
        const value = try Float32.read(stream, options.endian);
    ...
    pub fn write(stream: *BinaryStream, value: FloatTag, options: ReadWriteOptions) !void {
        if (options.tag_type) {
            try Int8.write(stream, @intFromEnum(TagType.Float));
        }
        ...
        try Float32.write(stream, value.value, .Little);
    }
};
